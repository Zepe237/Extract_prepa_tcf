...
def getPoint(i_file, class_name):
    ...
    for span_tag in span_tags:
        ...
        data = {
            'id': '1',
            'point': span_with_class['children'][0]['text']
        }
        results.append(data)
    #print(results)
    #print(len(results))
...
def getQuestion(dossier, class_name_to_extract, json_file, questions_file):
    ...
    for filename in os.listdir(dossier):
        ...
        if filename.endswith(".html"):  # Filtrer les fichiers texte
            ...
            tab_point = getPoint(full_path, 'lqc-available-points')
            ...
            with open(questions_file+str(cpt)+'.json', 'w', encoding='utf-8') as final_file:
                final_file.write('[')
                j = 0
                # Inserer les items de point dans ce for loop 
                for elt , point in zip(json_content, tab_point):
                    data =''
